...
def test_registration_existing_user(driver, create_user):
    ...
    # Ожидание: даем форме регистрации полностью раскрыться после клика
    WebDriverWait(driver, 5).until(
        expected_conditions.visibility_of_element_located(Input.INPUT_EMAIL))
    

    # ввод эмейла и паролей
    ...
    driver.find_element(*Input.INPUT_SECOND_PASSWORD).send_keys(
        user_data["password"]
    )
    ...
    error_red_email = driver.find_element(
        By.XPATH, "//input[@name='email']/parent::div"
    )
    border_color = error_red_email.value_of_css_property("border-top-color")
    assert (
        EXPECTED_ERROR_COLOR in border_color
    ), f"Цвет рамки не красный! Фактический цвет: {border_color}"

    error_red_password = driver.find_element(
        By.XPATH, "//input[@name='password']/parent::div"
    )
    border_color_password = error_red_password.value_of_css_property(
        "border-top-color"
    )
    assert (
        EXPECTED_ERROR_COLOR in border_color_password
    ), f"Цвет рамки не красный! Фактический цвет: {border_color_password}"

    error_red_submitPassword = driver.find_element(
        By.XPATH, "//input[@name='submitPassword']/parent::div"
    )
    border_color_submitPassword = error_red_submitPassword.value_of_css_property(
        "border-top-color"
    )
    assert (
        EXPECTED_ERROR_COLOR in border_color_submitPassword
